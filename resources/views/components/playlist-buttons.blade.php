...
@auth
    <button x-data="{ isFilled: '{{ auth()->user()->isInPlaylist($anime->id, App\Models\Anime::PLAYLIST_BACKLOG)}}' }" type="button"
            @click="isFilled = !isFilled; sendPlaylistRequest({{ $anime->id }}, {{ App\Models\Anime::PLAYLIST_BACKLOG }})"
    ...
    <button x-data="{ isFilled: '{{ auth()->user()->isInPlaylist($anime->id, App\Models\Anime::PLAYLIST_WATCHED)}}' }"
            type="button"
            @click="isFilled = !isFilled; sendPlaylistRequest({{ $anime->id }}, {{ App\Models\Anime::PLAYLIST_WATCHED }})"
            :class="[isFilled ? 'bg-green-500' : '']"
